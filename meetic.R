...
meetic <- function( sex, 
  ...
  if( sum( repro == "a" & popsurvival == 1 ) > 0 ){
    ...
    inherited_cloneline <- rep( popcloneline[ repro == "a" & popsurvival == 1 ], times = fecasex )
    inherited_X <- rep( popclonalorigin[ repro == "a" & popsurvival == 1, 1 ], times = fecasex )
    inherited_t <- rep( popclonalorigin[ repro == "a" & popsurvival == 1, 2 ], times = fecasex )
  }
  ...
  babycloneline <- c(rep(0, fec*sum( realized_mating) ), inherited_cloneline )
  babyclonalorigin <- matrix(0, nrow = length(babyrepro), ncol = 2)
  babyclonalorigin[,1] <-  c(rep(0, fec*sum( realized_mating) ), inherited_X )
  babyclonalorigin[,2] <-  c(rep(0, fec*sum( realized_mating) ), inherited_t )
