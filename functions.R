                    ...
                    run)
{
  ...
  TEMP <- diaspora( newbabyX = newbabyX, newbabyY = newbabyY, dispkernel = dispkernel, sex = sex, repro = repro )
  ...
  for ( t in 1: tps ){
    ...
    TEMP <- meetic( sex = sex, coordalive = coordalive, K = K, probamating = probamating, fec = fec, fecasex = fecasex, popgenome = popgenome, popsurvival = popsurvival, repro = repro, popXY = popXY, popX = popX, popY = popY, G = G )
    ...
    TEMP <- stillborn( babygenome = babygenome, babysex = babysex, babyX = babyX, babyY = babyY, B = B, M = M, Ka = Ka, bsline = bsline, pmut = pmut, babyrepro = babyrepro, G = G )
...
genesis <- function(K, 
  ...
  repro <- rep("s", Ninit)
  newbabyX <- rep( 0, Ninit )
  newbabyY <- rep( 0, Ninit )
  ...
  return( list( sex = sex, popgenome = popgenome, newbabyX = newbabyX, newbabyY = newbabyY, repro = repro ))
}
...
# input: newbabyX, newbabyY, dispersal kernel, sex, repro
# output: newpopX, newpopY, newRB, coordalive
...
diaspora <- function( newbabyX, 
  ...
  newpopY <- newbabyY + movB
  newpopX <- newbabyX + movR
...
# OUTPUT: babysex, babyX, babyY, babygenome, babyrepro
...
meetic <- function( sex, 
  ...
  return( list( babysex = babysex, babyX = babyX, babyY = babyY, babygenome = babygenome, babyrepro = babyrepro ) )
...
stillborn <- function( babygenome, 
  ...
  return( list( newbabygenome = newbabygenome, newbabysex = newbabysex, newbabyX = newbabyX, newbabyY = newbabyY, newbabyrepro = newbabyrepro ))
