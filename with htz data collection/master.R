master <- function( c,
  ...
  diversity_matrix <- matrix(nrow=1000, ncol=Xdim)
  htz_matrix <- matrix(nrow=1000, ncol=Xdim)
  sex_matrix <- matrix(nrow=1000, ncol=Xdim)
  ...
  while ( t < T2 ){
    ...
    if ( plot == TRUE & t/10 == round(t/10) ){

      namerun <- paste( compet, "K", K, '_fs', fec, '_fa',
                        fecasex, '_', probamating,'_G', G, '_b',
                        bsline, '_pm', pmut, '_d', mean_distance, '_c', c, '_',run,"_t", t,'_T.RData',
                        sep = "" )
